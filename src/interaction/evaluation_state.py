...
def check_resume_config(previous, current):
    ...
    keys = ("task", "model_path", "test_data_path", "image_root", "world_size", "latent_size",
            ...
            "ilvr_prompt_source_revision", "emma_prompt_source_revision", "zebra_protocol", "zebra_rule_ast_sha256", "data")
...
def load_rank_records(path, samples, task, rank_id, world_size, prompt_style, latent_size, repair_tail=False):
    ...
    for number, raw in enumerate(lines, 1):
        ...
        if task == "zebra" and (row.get("pid") != sample["pid"] or row.get("zebra_task") != sample["zebra_task"]
                                or row.get("gold") != sample["original_final_answer"]):
            raise ValueError(f"{path}:{number}: Zebra identity/answer mismatch")
