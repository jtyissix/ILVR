...
CUDA_VISIBLE_DEVICES=2 python -m src.interaction.evaluate \
  ...
  --test_data_path data/vsp_spatial_planning/test_direct.jsonl \
  --image_root data/vsp_spatial_planning \
  --output_dir outputs/eval_vsp_interaction_step2000 \
...
--output_dir outputs/eval_vsp_initial
```
...
模型仅接收 `text_input` 和初始地图 `image_input`，图像放在原题的 `<image>` 位置。
...
| `evaluation_config.json` | 参数、GPU 进程数、latent 步数、实际视觉后端、数据 SHA-256 与计数 |

```bash
python -m json.tool outputs/eval_vsp_interaction_step2000/metrics.json
```
...
```bash
python -m unittest tests.test_interaction_vsp tests.test_interaction_qwen -v
```
